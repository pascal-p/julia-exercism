...
# ╔═╡ b177e93f-6445-4532-aa8e-8c9587fc831e
begin
	using PlutoUI
	PlutoUI.TableOfContents(indent=true, depth=4, aside=true)
end

# ╔═╡ 62dcff31-a277-42a1-b993-6d40c582169f
using Distributions

# ╔═╡ 706e83ea-6a1f-4003-9c3e-403638209c9a
using Turing, MCMCChains

# ╔═╡ 82f940a5-bb46-4ffb-9908-c2a411d67ef6
using CairoMakie

# ╔═╡ 6afe4750-d68e-11ed-001a-7dccb494fd9a
md"""
## Coin toss experiment

ref. [https://www.youtube.com/watch?v=ixHKOz2vnoA](https://www.youtube.com/watch?v=ixHKOz2vnoA)
"""

# ╔═╡ a6fed666-cd48-41a9-a89d-a24b68885288
md"""Citation: [Ge et al. "Turing: A Language for Flexible Probabilistic Inference." Proceedings of the Twenty-First International Conference on Artificial Intelligence and Statistics, PMLR 84:1682-1690, 2018.](https://proceedings.mlr.press/v84/ge18b.html)"""

# ╔═╡ 4d8a89dc-99ff-4626-be15-197e2bd181d7
md"Tutorial Code Source (with some modifications): [Turing.jl Tutorials](https://turinglang.org/v0.24/tutorials/00-introduction/)"

# ╔═╡ 6664b143-63a7-4979-8b46-3d71ae426e19
md"""
### Make Observations
"""

# ╔═╡ ebef7944-5c5c-46fe-90af-42723640241a
const P = 0.5

# ╔═╡ 90cc2e43-28a4-41a2-999c-2e4000bfdfe5
n_tosses = 10_000

# ╔═╡ 71e57481-b38f-4fb0-9d61-20ef5b4a846b
data = rand(Distributions.Bernoulli(P), n_tosses)

# ╔═╡ 4743a835-b817-49d9-9353-3102c0fec957
p_data = sum(data) / n_tosses

# ╔═╡ b192a7ed-324d-4d03-8cb4-7d987a00f274
md"""
We are going to assume that the unknown proabability distribution is a beta distribution.
"""

# ╔═╡ 06ef2be9-f4ff-426e-a674-6dee9408addb
prior = Distributions.Beta(1, 1) # let's start with unifrom dist (= Beta(1, 1))

# ╔═╡ 71774e67-dab3-40a6-85c8-c571f47fec01
propertynames(prior)

# ╔═╡ 9e787db1-fab7-48d8-8351-dace822b79da
function update_prior(prior, data::Vector)
	n_heads = sum(data)
	n_tails = length(data) - n_heads
	Distributions.Beta(prior.α + n_heads, prior.β + n_tails)
end

# ╔═╡ 1c22a6c1-217a-41b8-9618-b3d0f64fb63d
update_prior(prior, data)

# ╔═╡ a9785d84-a04b-444a-85a6-1403260a935e
md"""
### Visualize the probability distributions
"""

# ╔═╡ 71f9c09b-568a-41a2-a7fc-6989d49797c8
md"Observation(s): $(@bind observation PlutoUI.Slider(1:50:n_tosses, 10, true))"

# ╔═╡ 3ec0b645-a31d-46a7-baf2-ed16ae6fb80c
update_prior(prior, data[1:observation])

# ╔═╡ 71668dbe-762d-4337-a04e-ba111c47b61f
v = Base.rand(update_prior(prior, data[1:observation]), 100_000)

# ╔═╡ fc9a290c-a513-483c-8547-b56988586e48
begin
	...
	ax₁ = CairoMakie.Axis(
		f₁[1, 1], 
		xticks = 0.0:0.01:1.0,
		title = "Coin tosses after $(observation) tosse(s)",
	...
	CairoMakie.vlines!(ax₁, p_data, color = :darkred)
	
	f₁
end

# ╔═╡ 45d365fb-3325-4e8c-9ab4-965d6fbef3f8
md"""
## Using `Turing.jl`

### Defining a model base on the data
"""

# ╔═╡ 1d167fec-78f7-4333-bc79-912465d1c8e1
@model function cointoss(; n_tosses)
	p ~ Distributions.Beta(1., 1.)  # prior distribution 
	y ~ Turing.filldist(Distributions.Bernoulli(p), n_tosses)
	y
end

# ╔═╡ d57d56d4-5f3c-47b3-bcf5-4f0f02aa65b5
cointoss(y) = cointoss(; n_tosses = length(y)) | (; y)  # conditioned on y (observations)

# ╔═╡ c0f1c1f9-f785-47c7-bb09-cf623bfb7dc4
rand(cointoss(; n_tosses))  # non conditioned function call

# ╔═╡ eb6ac26d-18b8-415b-9c4e-a1309071cd95
model = cointoss(data)

# ╔═╡ d1a90ff0-7cc1-4dee-8b2c-7a1c32fe5ed8
md"""
### Approximate Posterior Distribution
"""

# ╔═╡ 2cd6e2af-358c-49ad-9074-0419d056d7a1
sampler = Turing.NUTS()  # No U-Turn Sampler

# ╔═╡ 572331eb-97ef-47e6-87a5-8d9e4df3f17e
md"Samples: $(@bind samples PlutoUI.Slider(50:10:2_000, 50, true))"

# ╔═╡ 9698fdf4-e638-459a-b914-569e4dff3a58
chain = Turing.sample(model, sampler, samples; progress=true)

# ╔═╡ 5392981e-5c72-4705-a41d-54640630672e
propertynames(chain)

# ╔═╡ 6312a4d6-b37a-4b37-8978-9575b10a2a7e
chain.value

# ╔═╡ 5eb785a3-ef3e-42da-bd3d-bced38bf81e4
chain.value[1:50, 1, 1]

# ╔═╡ 8736d2b4-2b3e-47aa-a898-a0ff421b2ee1
sum_stats = Turing.summarystats(chain)

# ╔═╡ 0d595611-bfdc-48c9-8279-5d921883fd00
μ = sum_stats[1, :mean]

# ╔═╡ f21f2bc7-f094-465e-96e5-70b145cfa412
md"""
### Visualize the Probability Distribution
"""

# ╔═╡ 721218e0-356c-4c52-b462-b7157cf3b869
# Turing.plot(chain)

# ╔═╡ 1040fc43-bb2b-4484-b744-759c96594c66
begin
	f₂ = CairoMakie.Figure(resolution=(600, 400), fontsize=11)
	...
	CairoMakie.vlines!(ax₂, μ, color = :darkred)
	
	f₂
end

# ╔═╡ fe41e5e5-ae41-4f95-be8a-a7c52a595bf3
begin
	f₃ = CairoMakie.Figure(resolution=(600, 400), fontsize=11)
	...
	CairoMakie.lines!(
		ax₃, 
        pdf.(update_prior(prior, data[1:observation]), 0:0.01:1),
        normalization=:pdf,
...
# ╔═╡ 8479f1ba-143f-413c-a25a-0a3d1148336d


# ╔═╡ 080ee0ad-76e1-4e69-b963-0d552fb4c793
html"""
<style>
  main {
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "63f2247de002da8dc45b2e68ee718ca73ec4d267"

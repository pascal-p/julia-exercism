...
@testset "initialisation" begin
  layer_dims = Int32[5, 4, 3]
  ann = ANN{Float32}(layer_dims; seed=70)


  @test size(ann.parms["w1"]) == (layer_dims[2], layer_dims[1])
  @test ann.parms["w1"] ≈ Float32[ 0.318851   0.439791   0.244843   0.56474   0.0140048;
  ...
  @test size(ann.parms["w2"]) == (layer_dims[3], layer_dims[2])
  @test ann.parms["w2"] ≈ Float32[ 0.356487  0.0401505  0.251124  0.0910389;
                               0.44893   0.491702   0.152735  0.251607;
                               0.481913  0.0862954  0.273743  0.669299 ]

  @test size(ann.parms["b1"]) == (layer_dims[2], 1)
  @test ann.parms["b1"] ≈ Float32[0.0; 0.0; 0.0; 0.0]

  @test size(ann.parms["b2"]) == (layer_dims[3], 1)
  @test ann.parms["b2"] ≈ Float32[0.0; 0.0; 0.0]
end
...
@testset "linear_forward/1" begin
  ...
  ntuple = linear_forward(a, ann.parms["w1"], ann.parms["b1"])
...
@testset "linear_forward_sigmoid / 2" begin
  ...
  ntuple = linear_forward_activation(a, ann.parms["w1"], ann.parms["b1"];
...
@testset "forward_pass" begin
  parms = Dict{String, Array{Float32, N} where N}(
               "w1" => Float32[ 0.35480861  1.81259031 -1.3564758 -0.46363197  0.82465384;
...
@testset "update_ann_model" begin
  ...
  ∇ = Dict{String, Array{Float32, N} where N}(
           "∂w1" => Float32[ 1.78862847  0.43650985  0.09649747 -1.8634927;
  ...
  ann = ANN{Float32}(parms; η=0.1)
  update_ann_model!(ann, ∇)

  @test ann.parms["w1"] ≈ Float32[-0.59562069 -0.09991781 -2.14584584  1.82662008;
                                  -1.76569676 -0.80627147  0.51115557 -1.18258802;
                                  -1.0535704  -0.86128581  0.68284052  2.20374577]
  @test ann.parms["b1"] ≈ Float32[ -0.04659241;
                                   -1.28888275;
                                   0.53405496 ]
  @test ann.parms["w2"] ≈ Float32[ -0.55569196  0.0354055   1.32964895 ]
  @test ann.parms["b2"] ≈ Float32[ -0.84610769 ]
end

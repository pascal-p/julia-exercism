...
@testset "DSP 4v_5e" begin
  dsp = shortest_path("$(FILEDIR)/simple_4v_5e_digraph.txt", 1)

  @test has_path(dsp, 4) # true
  @test path_to(dsp, 4) == [1, 2, 3, 4]
  @test dist_to(dsp, 4) == 6
end

@testset "Exceptions on 4v_5e" begin
  src = 10

  @test_throws AssertionError dsp = shortest_path("$(FILEDIR)/simple_4v_5e_digraph.txt", 10)
end

@testset "DSP 8v_13e" begin
  dsp = shortest_path("$(FILEDIR)/simple_8v_13e_digraph.txt", 6)

  @test has_path(dsp, 7) # true
  @test path_to(dsp, 7) == [6, 2, 4, 7]
  @test dist_to(dsp, 7) == 113

  @test has_path(dsp, 3) # true
  @test path_to(dsp, 3) == [6, 8, 3]
  @test dist_to(dsp, 3) == 62

  @test has_path(dsp, 1) # true
  @test path_to(dsp, 1) == [6, 5, 1]
  @test dist_to(dsp, 1) == 73
end

@testset "DSP 8e_16v" begin
  dsp = shortest_path("$(FILEDIR)/8v_16e.txt", 1)

  for (v, d) ∈ [(1, 0), (2, 1), (3, 2), (4, 3), (5, 4), (6, 4), (7, 3), (8, 2) ]
    @test dist_to(dsp, v) == d
  end
end

@testset "DSP 200v_3734e challenge" begin
  dsp = shortest_path("$(FILEDIR)/200e_3734v.txt", 1)
  ...
  e = 161
  @test ix_max == e
  @test dist_to(dsp, e) == 4772

  exp_path = [1, 80, 19, 88, 161]
  @test path_to(dsp, e) == exp_path
  @test sum_w(dsp, exp_path) == dsp.dist_to[e]
...
# for x ∈ [7,37,59,82,99,115,133,165,188,197]
#   println("==> path to: $(x) from 1 is $(path_to(dsp, x))")
# end

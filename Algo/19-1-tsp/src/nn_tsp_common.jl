...
"""
function tsp(infile::String, DT::DataType; x_sorted=false, no_index=false)
...
"""
@inline function min_dist(vcities::Vector{Tuple{Integer, Point{T}}}, DT::DataType, ix::Integer,
                  hvisited::Dict{T1, Bool}, hsh::Dict{T1, T1}) where T
  e_mdist, m_ix = typemax(DT), zero(T1)
...
@inline function min_dist(vcities::Vector{Point{T}}, DT::DataType, ix::Integer, n::Integer,
  ...
  for jx ∈ T1(2):n
    ...
    x_cdist = DT(vcities[ix].x - vcities[jx].x)
    x_cdist *= x_cdist                     ## calc. x-dist (last visited city, next city to visit)

    x_cdist > e_mdist && break             ## as vcities is x-coord. sorted in increasing order
    #                                      ## no need to go further when x-dist is greater than the recorded min. dist

    y_cdist = DT(vcities[ix].y - vcities[jx].y)
    e_cdist = x_cdist + y_cdist * y_cdist  ## square euclidean distance (last visited city, next city to visit)

...
@testset "on g1.txt" begin
  src = 1
  T = Int
  bfsp = BFSP{T, T}("$(TF_DIR)/g1.txt", src, YA_BFSP.EWDiGraph{T, T};
...
@testset "on g2.txt" begin
  src = 1
  T, T1 = Int, Float32
  bfsp = BFSP{T, T1}("$(TF_DIR)/g2.txt", src, YA_BFSP.EWDiGraph{T, T1};
                     positive_weight=false)
...
@testset "on g3.txt with src ≡ 1" begin
  src = 1
  T = Int
  bfsp = BFSP{T, T}("$(TF_DIR)/g3.txt", src, YA_BFSP.EWDiGraph{T, T};
                    positive_weight=false)
  ...
  @test path_to(bfsp, 25) == [1, 392, 490, 324, 511, 620, 679, 806, 841, 629, 129, 828, 166, 25]
  @test path_to(bfsp, 999) == [1, 392, 490, 324, 511, 541, 672, 534, 739, 930, 999]
  @test path_to(bfsp, 513) == [1, 392, 490, 324, 680, 584, 658, 902, 459, 527, 761, 367, 7, 508, 513]
  @test path_to(bfsp, 994) == [1, 392, 490, 324, 511, 620, 679, 806, 841, 963, 998, 515, 289, 566, 709, 766, 994]
...
@testset "on g3.txt with src ≡ 399" begin
  ...
  @test path_to(bfsp, 904) == [399, 175, 177, 250, 150, 360, 211, 148, 426, 517, 470, 861, 630, 604, 869, 292, 454, 736, 771, 810, 895, 904]
  # [(399, 175), (175, 177), (177, 250), (250, 150), (150, 360), (360, 211), (211, 148), (148, 426), (426, 517), (517, 470), (470, 861), (861, 630), (630, 604), (604, 869), (869, 292), (292, 454), (454, 736), (736, 771), (771, 810), (810, 895), (895, 904)]
